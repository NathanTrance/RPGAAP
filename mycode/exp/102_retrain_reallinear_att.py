...
from utils.util import MyProgressBar, fix_seed, format_time, cal_binary_metrics
...
def create_projection_matrix(m, d, seed=0, scaling=0, struct_mode=False):
    ...
    if remaining_rows > 0:
        ...
        if struct_mode:
            ...
            unstructured_block = torch.randn((d, d))
            q, _ = torch.qr(unstructured_block)  # QR分解，q为正交矩阵
            q = torch.t(q)  # 转置
        block_list.append(q[0:remaining_rows])  # 取前remaining_rows行
    final_matrix = torch.vstack(block_list)

    current_seed += 1
    torch.manual_seed(current_seed)
    if scaling == 0:
        multiplier = torch.norm(torch.randn((m, d)), dim=1)  # 从正态分布中采样
    elif scaling == 1:
...
def softmax_kernel_transformation(data, is_query, projection_matrix=None, numerical_stabilizer=0.000001):
    ...
    # 应用缩放的 softmax 操作，对查询和键/值的处理不同。
    if is_query:
        # 为了数值稳定性减去最大值，并添加一个小常数以避免除以零。 这里就是eq.4 但是经过变形，ratio是根号m
        data_dash = ratio * (
                torch.exp(data_dash - diag_data - torch.max(data_dash, dim=last_dims_t, keepdim=True)[
                    0]) + numerical_stabilizer
        )
    else:
        # 减去两个维度上的最大值以增加稳定性，并添加一个小常数。
        data_dash = ratio * (
...
def kernelized_softmax(query, key, value, projection_matrix=None,
    ...
    query_prime = query_prime.permute(1, 0, 2, 3) # [N, B, H, M] # 这里为什么要转置，不转是否可以, #测试后发现是可以的, 只是计算过程会让人非常难以理解
...
class SAGE(nn.Module):
    def __init__(self, d_in, n_classes=2, **kwargs):
        ...
        if use_dyple and preprocess == 'None':
            ...
            self.embedding0 = nn.Sequential(
                nn.Linear(d_in, d_hidden*8),
                self.act,
                nn.BatchNorm1d(d_hidden*8) if gnn_use_bn else None,
                nn.Linear(d_hidden*8, d_hidden),
                self.act,
        ...
        if self.use_mha:
            ...
            # self.Wk = nn.Linear(d_hidden, d_hidden)
            # self.Wq = nn.Linear(d_hidden, d_hidden)
            # self.Wv = nn.Linear(d_hidden, d_hidden)
    ...
    def forward(self, blocks, feat):
        ...
        if self.use_mha:
            ...
            M, dim = q.shape[0], q.shape[1]
            N = v.shape[0]

            query = q.reshape(-1, M, 1, dim)
            key = k.reshape(-1, N, 1, dim)
            value = v.reshape(-1, N, 1, dim)

            seed = torch.ceil(torch.abs(torch.sum(query) * BIG_CONSTANT)).to(torch.int32)
            projection_matrix = create_projection_matrix(
                50, dim, seed=seed).to(query.device)
            o = kernelized_softmax(query, key, value, projection_matrix)
            o = o.reshape(M, -1)
            # o, _ = self.mha(q, k, v)
            # o = self.act(o)
...
class Experiment:
    ...
    def train(self):
        ...
        self.trainer = Trainer(
            ...
            # check_val_every_n_epoch=5,  # 大数据集每5个epoch进行一次验证
            # num_sanity_val_steps=0 # 取消sanity check
        )
...
# run_one_exp
@hydra.main(config_path="../../config/SAGE_MiniF_DyPLE_MHA", config_name="tsocial.yaml", version_base=None)
